...
\begin{soln}
	\begin{enumerate}
		\item Google Searches (AI Overview).
		\item ChatGPT to generate and brainstorm ideas.
		\item COSC 320 Lecture Slides/Notes/Handouts.
	\end{enumerate}
\end{soln}
...
% Follow the steps below to show how to reduce the TG problem to the SAT problem. That is, given an instance 
% $K[1..n][1..n]$ of the TG problem, construct an instance $I$ of SAT, such that instance $K$ is a 
% Yes-instance of TG if and only if $I$ is a Yes-instance of SAT.

% Your reduction will have three variables, $x_{i,1}$, $x_{i,2}$, and $x_{i,3}$ for each student $i$. 
% The rough idea is that if there is a satisying assignment to $I$ in which $x_{i,1}$ is true, 
% then student $i$ is in group 1; if $x_{i,2}$ is true, then student $i$ is in group 2; and 
% if $x_{i,3}$ is true, then student $i$ is in group 3.
...
\For{each group $G_i$}
	...
	\For{$i$ from 1 to $n$}
		...
		\EndIf
	\EndFor
	\For{each member $i$ in $M$}
		\For{each member $j$ in $M$}
			\If{$(i \neq j \text{ and } K[i, j] == 1)$}
				\Return "Check fails"
			\EndIf 
		\EndFor 
	\EndFor 
